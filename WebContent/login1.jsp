...
/*会员注册*/
.login ul{/*background:url(${ctx }/images/line.png) repeat-x;*/ padding-top:10px; border-top:1px solid #fff}
...
.l_button,.r_button{background:url(${ctx }/images/login_button.png) no-repeat; width:118px; height:39px; border:none; cursor:pointer; display:block; float:left; text-indent:-9000px}
...
.onShow,.onFocus,.onError,.onCorrect,.onLoad{background:url(${ctx }/images/reg_bg.png) no-repeat 3000px 3000px;padding-left:30px; font-size: 12px; height:25px; width:124px; display:inline-block; line-height:25px; vertical-align:middle; overflow:hidden; margin-left:6px}
...
.id input,.pw input,.in_id,.in_mo,.reg_input,.reg_input_pic{_behavior:url(${ctx }/js/Round_htc.htc);-moz-border-radius:4px;-webkit-border-radius:4px;border-radius:4px; height:25px;}
.user_button a,.pay_but{_behavior:url(${ctx }/js/Round_htc.htc);-moz-border-radius:100px;-webkit-border-radius:100px;border-radius:100px;}
...
<script src="${ctx }/js/jquery-1.2.6.min.js" type=text/javascript></script>
<script src="${ctx }/js/formValidator_min.js" type="text/javascript" charset="UTF-8"></script>
...
$(document).ready(function(){
	...
	$("#iptCard").formValidator({onshow:"15位或18位的身份证",onfocus:"15位或18位的身份证",oncorrect:"输入正确"}).functionValidator({fun:isCardID});
...
function test1(obj)
{
	if(obj.value=="全角字符当作1个长度")
	{
		$.formValidator.getInitConfig("1").wideword = false;
		obj.value = "全角字符当作2个长度";
	}
	else
	{
		$.formValidator.getInitConfig("1").wideword = true;
		obj.value = "全角字符当作1个长度";
	}
...
<script type="text/javascript">
		...
		function checkExist(){
			$.get("${ctx }/loginuser/checkname",{'t_UserName':$('#t_UserName').val()}, function(data, status) {
